...
return {
  ...
  dependencies = {
    ...
    { "echasnovski/mini.pick", config = true },
    {
      ...
      version = "*",
      build = "pipx list | grep -q 'vectorcode' && pipx upgrade vectorcode || pipx install vectorcode",
      dependencies = { "nvim-lua/plenary.nvim" },
    },
  },
  -- NOTE: Recall that when you manually define the config function for a plugin in lazy.nvim, you take full control of its setup process. lazy.nvim will not automatically call require('plugin-name').setup(opts).
  -- If you don't define 'config', then opts *will be passed to require('plugin-name').setup automatically.
    ...
    adapters = {
      tavily = function()
        return require("codecompanion.adapters").extend("tavily", {
          env = {
            api_key = "cmd: cat ~/.secrets/code_companion/tavily.token.txt",
          },
        })
      end,
    },
...
-- These are tehe default settings at the time of writing
-- They will not be applied here by default, but can be used as a reference
